...
// ImportIfEmpty imports the chapter and members if database is empty
func ImportIfEmpty(ctx context.Context, pool *pgxpool.Pool) error {
	// Check if chapters table is empty
	var count int
	err := pool.QueryRow(ctx, `SELECT COUNT(*) FROM chapters`).Scan(&count)
	if err != nil {
		return fmt.Errorf("check chapters: %w", err)
	}

	if count > 0 {
		// Database already has data, skip seed
		return nil
	}
	...
	for idx, m := range members {
		...
		// Create member
		displayID := fmt.Sprintf("ΤΣΣ-%03d", idx+1)
