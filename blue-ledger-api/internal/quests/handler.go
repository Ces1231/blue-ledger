...
// Handler handles all quests HTTP endpoints.
type Handler struct {
	svc Service
}

// NewHandler creates a new quests handler.
func NewHandler(svc Service) *Handler {
	return &Handler{svc: svc}
}

// RegisterRoutes mounts quest routes on the given Echo group.
...
func (h *Handler) List(c echo.Context) error {
	...
	if err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to list quests")
	}
...
func (h *Handler) GetByID(c echo.Context) error {
	...
	if err != nil {
		...
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to get quest")
	}
...
func (h *Handler) GetProgress(c echo.Context) error {
	...
	p, err := h.svc.GetProgress(c.Request().Context(), memberID, c.Param("id"))
	if err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to get progress")
	}
	return c.JSON(http.StatusOK, map[string]any{"data": p})
}
...
func (h *Handler) Create(c echo.Context) error {
	...
	if err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to create quest")
	}
...
func (h *Handler) Update(c echo.Context) error {
	...
	if err != nil {
		...
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to update quest")
	}
	return c.JSON(http.StatusOK, map[string]any{"data": item})
}
